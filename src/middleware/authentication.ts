...
export async function authentication(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    || (token === CONFIG.STATIC_TOKEN && requestMethod === 'POST' && requestUrl && CONFIG.mutliPOSTRouteModules.indexOf(requestUrl) !== -1)) {
      ...
      return res.status(401).send(new UnauthorizedError("Unauthenticated! Please provide valid token", {
    ...
    return res.status(401).send(new UnauthorizedError("Session expired! Please Login again", {
